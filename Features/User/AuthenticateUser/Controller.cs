namespace Meetups.Features.User.AuthenticateUser;
...
public class Controller : ApiControllerBase
{
    ...
    {
        var user = await Context.Users.SingleOrDefaultAsync(user => user.Username == requestDto.Username);
        if (user is null)
        {
            return NotFound();
        }

        var passwordMatches = BCrypt.Verify(requestDto.Password, user.Password);
        if (!passwordMatches)
        {
            return Conflict();
        }
